...
c$$$c angle cuts & energy cuts for at least one leptons
c$$$            if(
c$$$     $         (thetax(p(0,i)) .gt. thetaxlmins).and.
c$$$     $         (thetax(p(0,i)) .lt. thetaxlmaxs).and.
c$$$     $         (thetay(p(0,i)) .gt. thetaylmins).and.
c$$$     $         (thetay(p(0,i)) .lt. thetaylmaxs).and.
c$$$     $         (theta(p(0,i)) .gt. thetalmins).and.
c$$$     $         (theta(p(0,i)) .lt. thetalmaxs).and.
c$$$     $         (p(0,i) .gt. elmins).and.
c$$$     $         (p(0,i) .lt. elmaxs)) then
c$$$               tripass=.true.               
c$$$c               write (47,*) "PASS!"
c$$$            endif
c$$$
c$$$
c$$$c there is only one positron; check if it passes angle cuts & energy cuts
c$$$            if(
c$$$     $         is_a_lp(i).and.
c$$$     $         (thetax(p(0,i)) .gt. thetaxlminsp).and.
c$$$     $         (thetax(p(0,i)) .lt. thetaxlmaxsp).and.
c$$$     $         (thetay(p(0,i)) .gt. thetaylminsp).and.
c$$$     $         (thetay(p(0,i)) .lt. thetaylmaxsp).and.
c$$$     $         (theta(p(0,i)) .gt. thetalminsp).and.
c$$$     $         (theta(p(0,i)) .lt. thetalmaxsp).and.
c$$$     $         (p(0,i) .gt. elminsp).and.
c$$$     $         (p(0,i) .lt. elmaxsp)) then
c$$$               passlp=.true.
c$$$               nlppass=nlppass+1
c$$$               jpasslp=i
c$$$            endif

c$$$c there are two electrons; check if one passes angle cuts & energy cuts
c$$$
c$$$            if(
c$$$     $         is_a_lm(i).and.
c$$$     $         (thetax(p(0,i)) .gt. thetaxlminsp).and.
...
c$$$            endif
c$$$            
c$$$c...        Evaluate electron-positron criteria
c$$$            ! only one electron passes the angle & energy cuts
c$$$            if(nlppass.eq.1.and.nlmpass.eq.1.and.(.not.pairpass)) then
c$$$
c$$$              if(
c$$$     $         (dSqrt(Sumdot(p(0,jpasslp),p(0,jpasslm(1)),+1d0)).gt.mmllminsp)
c$$$     $         .and.
c$$$     $         (dSqrt(Sumdot(p(0,jpasslp),p(0,jpasslm(1)),+1d0)).lt.mmllmaxsp)
c$$$     $         .and.
c$$$     $         (p(0,jpasslp)+p(0,jpasslm(1)).gt.eltotsp)
c$$$     $          ) then
c$$$                 pairpass=.true.
c$$$               endif 
c$$$             ! two electrons both pass the angle & energy cuts  
c$$$             elseif(nlppass.eq.1.and.nlmpass.eq.2.and.(.not.pairpass)) then
c$$$              ! loop over the two electrons
c$$$              do kk=1,2
c$$$               if(
c$$$     $          (dSqrt(Sumdot(p(0,jpasslp),p(0,jpasslm(kk)),+1d0)).gt.mmllminsp)
c$$$     $          .and.
c$$$     $          (dSqrt(Sumdot(p(0,jpasslp),p(0,jpasslm(kk)),+1d0)).lt.mmllmaxsp)
c$$$     $          .and.
c$$$     $          (p(0,jpasslp)+p(0,jpasslm(kk)).gt.eltotsp)
c$$$     $          ) then
c$$$                 pairpass=.true.
c$$$                endif
c$$$              enddo ! Close loop over the two electrons              
c$$$             endif ! Close the pair case           
c$$$
...
c$$$      ! at least one lepton criteria
c$$$      if(.not.tripass) then
c$$$       passcuts=.false.
c$$$       return
c$$$      endif

c$$$      ! Positron criteria
c$$$      if(.not.passlp) then
c$$$       passcuts=.false.
c$$$       return
c$$$      endif
         
c$$$      ! At least one electron criteria
c$$$      if(.not.passonelm) then
c$$$       passcuts=.false.
c$$$       return
c$$$      endif
c$$$
