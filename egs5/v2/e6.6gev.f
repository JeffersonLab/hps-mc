...
!     ----------------------
      ...
      real qpart, ppart, am
      common /tmpart/ npart, qpart(1000), ppart(4,1000)
...
190   format(/,' Shower Results:',///,7X,'e',14X,'z',14X,'w',10X,
     1   'iq',3X,'ir',2X,'iarg',/)
      ...
      do i=1,ncases
        ...
        if(npart.eq.0) then
           write(51,777) 1,0.,0.,0.,0.001,0.001
        end if

        if(npart.ge.1) then
           write(51,777) npart,(qpart(j),(ppart(k,j),k=1,4),j=1,npart)
 777       format(i5,/,(2x,f3.0,4f11.7))
        end if
...
cx      end if
      ...
      if(ir(np).eq.3) then
         ...
         if(iq(np).eq.-1) then
            if(e(np).lt.10.0) iout = 0
c e > 0.6 * Ebeam and theta < 15 mrad
            if(e(np).gt.3960.0.and.
