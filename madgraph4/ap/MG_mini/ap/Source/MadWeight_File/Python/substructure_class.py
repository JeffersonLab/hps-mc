...
class ECS_sector(diagram,Level_generation):
    """ The ECS sector.
    ...
    def define_blob(self,diag,propa_in_ecs=""):
        ...
        for i in range(0,self.intrinsec):
            #check if all mother exist
            try:
                propa_in_ecs[i].all_mother
            except:
                propa_in_ecs[i].def_all_mother()
...
class blob_sector(diagram):
    ...
    def __init__(self,particle,diag,ECS_sec=''):
        ...
        if diag.blob_content.has_key(particle.MG): # check if the blob is already defined
            ...
            if particle.channel!='S': #remove T channel from creating a blob
                    return None    
