...
c.....first boost
     ...
     $        - sqrt(ebeam(2)**2-mbeam(2)**2)
      ...
      enddo
c e+
      if(plab(0,5) .lt. ef .or. 
     -     thetay(plab(0,5)) .lt. thetayfmin) then
         passcuts=.false.
         return
      end if
c e-
      itvm3 = 0
      if(plab(0,3) .gt. ef .and. 
     -     thetay(plab(0,3)) .gt. thetayfmin) then
         itvm3 = 1
      end if
      itvm6 = 0
      if(plab(0,6) .gt. ef .and. 
     -     thetay(plab(0,6)) .gt. thetayfmin) then
         itvm6 = 1
      end if
      if(itvm3.eq.0.and.itvm6.eq.0) then
         passcuts=.false.
         return
      endif
      am12= 0.0
      am23= 0.0
      if(itvm3.eq.1) am12=dSqrt(Sumdot(plab(0,5),plab(0,3),+1d0))
      if(itvm6.eq.1) am23=dSqrt(Sumdot(plab(0,5),plab(0,6),+1d0))

      if(am12.lt.mmff .and. am23.lt.mmff) then
         passcuts=.false.
         return
      endif
...
c               return
c            endif
c            if(thetay(plab(0,i)) .lt. thetayfmin) then
...
c                  return
c               endif
c               if(dSqrt(Sumdot(plab(0,i),plab(0,jf),+1d0)).gt.mmffmax)then
