...
c-charged-leptons
         if (idup(i,1).eq.11)  is_a_fm(i)=.true. ! e+  e-
...
c- F distinguished leptons
         ...
         if (idup(i,1).eq.611)  is_a_fm(i)=.true. ! f+ f-
         if (idup(i,1).eq.-611)  is_a_fp(i)=.true. ! f+ f-
c-b-quarks
...
c
      do i=nincoming+1,nexternal-1
         do j=i+1,nexternal
            ...
            if(do_cuts(i).and.do_cuts(j)) then
               ...
               if( is_a_f(i).and.is_a_f(j) ) s_min(j,i)=mmff*dabs(mmff)     
     ...
     &            s_min(j,i)=mmll*dabs(mmll)  !only on l+l- pairs (same flavour) 
               ...
               if(is_a_f(i).and.is_a_f(j) ) s_max(j,i)=mmffmax*dabs(mmffmax)
               if((is_a_l(i).and.is_a_l(j)).and.
     &            (abs(idup(i,1)).eq.abs(idup(j,1))).and.
     &            (idup(i,1)*idup(j,1).lt.0)) 
