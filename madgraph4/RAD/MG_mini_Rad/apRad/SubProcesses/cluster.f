...
c**************************************************************************
      ...
      double precision minpt2ij,pt2ij(n_max_cl)
      ...
      logical findmt
      external findmt
      double precision dj, djb, dot, SumDot
      external dj, djb, dot, SumDot
...
c     check whether 2->(n-1) process w/ cms energy > 0 remains
                     iwinp=imap(3-j,2);
c                     do k=0,3
c                        pcl(k,idij)=pcl(k,idj)-pcl(k,idi)
cc                        pcmsp(k)=pcl(k,idij)+pcl(k,iwinp)
c                     enddo
