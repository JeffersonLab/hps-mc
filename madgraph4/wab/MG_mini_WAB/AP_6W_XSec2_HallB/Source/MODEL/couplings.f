...
c      following line added by Sebouh
      double precision fel, fqu, qr
      double precision myelmass 
...
c
      ...
      myelmass=0.0005111
      Anuc = 184
      aval = 111/( myelmass*Znuc**(1/3) )
      dval = 0.164/Anuc**(2/3)
      apval = 773/( myelmass*Znuc**(2/3) ) 
      tval = -(pp(0,NucI)-pp(0,NucO))**2+(pp(1,NucI)-pp(1,NucO))**2+
     $         (pp(2,NucI)-pp(2,NucO))**2+(pp(3,NucI)-pp(3,NucO))**2
      
      
c      fullcoupling = ee * (Znuc**2 *aval**4 *
c     $    tval**2/((1+aval**2*tval)*(1+tval/dval))**2 +
c     $    Znuc * apval**4 * tval**2 * (1+1.9276*tval)**2 /
c     $    ((1+apval**2*tval)*(1+1.40845*tval)**4)**2 )**0.5


c     elastic part of the form factor (original)
c      fel = aval**2 *                                                                                                          
c     $    tval/((1+aval**2*tval)*(1+tval/dval))
c     modified elastic part of form factor 
      qr = tval**.5*6.87/.197
      fel = 3/qr**3*(sin(qr)-qr*cos(qr))

c            Z= 74;
c            double r = 6.87;
c            double x = Math.sqrt(Q2*r*r/(hbarc*hbarc));
c            double F_calc = 3/(x*x*x)*(Math.sin(x)-x*Math.cos(x));


      
c     quasielastic part
      fqu = apval**2 * tval * (1+1.9276*tval) /
     $    ((1+apval**2*tval)*(1+1.40845*tval)**4)
      fullcoupling = ee * (Znuc**2 *fel*fel +
     $    Znuc * fqu**2 )**0.5
