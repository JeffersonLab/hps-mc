...
C
      ...
      logical xgoodf,passfp,passonefm
      Real*8 efRatVal
      integer jf
...
C $B$ TO_SPECISA $B$ !this is a tag for MadWeight
      ...
      LOGICAL  IS_A_F(NEXTERNAL),IS_A_FP(NEXTERNAL),IS_A_FM(NEXTERNAL)
      COMMON /TO_SPECISA/IS_A_J,IS_A_A,IS_A_L,IS_A_B,IS_A_NU,IS_HEAVY,
     . IS_A_ONIUM, IS_A_F, IS_A_FP, IS_A_FM
...
c      integer nfpass
      integer nfppass,nfmpass,k,jpassfp,jpassfm(2)
c      logical onepass
c      logical passfp,passonefm,pairpass
      logical pairpass
c      logical pairpass
c      integer jpass(4), 
c      integer k, jpassfp, jpassfm(2)
...
c... nincoming is # of incoming particles (usually 2), nexternal is total # 
         if(is_a_f(i)) then
...
c there is only one fp; check if it passes cuts

            if(
     $         is_a_fp(i).and.
     $         (thetax(plab(0,i)) .gt. thetaxfmin).and.
     $         (thetax(plab(0,i)) .lt. thetaxfmax).and.
     $         (thetay(plab(0,i)) .gt. thetayfmin).and.
     $         (thetay(plab(0,i)) .lt. thetayfmax).and.
     $         (theta(plab(0,i)) .gt. thetafmin).and.
     $         (theta(plab(0,i)) .lt. thetafmax).and.
     $         (plab(0,i) .gt. ef).and.
     $         (plab(0,i) .lt. efmax)) then
               passfp=.true.
               nfppass=nfppass+1
               jpassfp=i
            endif

c there are two fm's; check if one passes cuts
...
c            onepass=.true.
cc...        Does it pass exclusive criteria?
c            if(plab(0,i) .lt. ef) then
c               if(debug) write(*,*) "f- too soft -> fails"
c               onepass=.false.
c            elseif(plab(0,i) .gt. efmax) then
c               if(debug) write(*,*) "f- too hard -> fails"
c               onepass=.false.
c            elseif(theta(plab(0,i)) .lt. thetafmin) then
c               if(debug) write(*,*) "f small angle -> fails"
c               onepass=.false.
c            elseif(theta(plab(0,i)) .gt. thetafmax) then
c               if(debug) write(*,*) "f large angle -> fails"
c               onepass=.false.
c            elseif(thetax(plab(0,i)) .lt. thetaxfmin) then
c               if(debug) write(*,*) "f small x-angle -> fails"
c               onepass=.false.
c            elseif(thetax(plab(0,i)) .gt. thetaxfmax) then
c               if(debug) write(*,*) "f large x-angle -> fails"
c               onepass=.false.
c               return
c            elseif(thetay(plab(0,i)) .lt. thetayfmin) then
c               if(debug) write(*,*) "f small y-angle -> fails"
c               onepass=.false.
c            elseif(thetay(plab(0,i)) .gt. thetayfmax) then
c               if(debug) write(*,*) "f large y-angle -> fails"
c               onepass=.false.
c            endif
c
c            if(onepass) then
c               nfpass=nfpass+1
c               if(debug) write(*,*) nfpass," pass so far: ", i 
c            else
c               goto 666
c            endif
...
c...        Evaluate pair. criteria            
            if(nfppass.eq.1.and.nfmpass.eq.1.and.(.not.pairpass)) then
              if(
     $         (dSqrt(Sumdot(plab(0,jpassfp),plab(0,jpassfm(1)),+1d0)).gt.mmff)
     $         .and.
     $         (dSqrt(Sumdot(plab(0,jpassfp),plab(0,jpassfm(1)),+1d0)).lt.mmffmax)
     $         .and.
     $         (plab(0,jpassfp)+plab(0,jpassfm(1)).gt.efTot)
     $          ) then
                 pairpass=.true.
               endif
             elseif(nfppass.eq.1.and.nfmpass.eq.2.and.(.not.pairpass)) then
              do k=1,2
               if(
     $          (dSqrt(Sumdot(plab(0,jpassfp),plab(0,jpassfm(k)),+1d0)).gt.mmff)
     $          .and.
     $          (dSqrt(Sumdot(plab(0,jpassfp),plab(0,jpassfm(k)),+1d0)).lt.mmffmax)
     $          .and.
     $          (plab(0,jpassfp)+plab(0,jpassfm(k)).gt.efTot)
     $          ) then
                 pairpass=.true.
                endif
              enddo               
      ...
      enddo ! Close loop over final state particles
              
c            jpass(nfpass)=i
c            if(onepass.and.nfpass.gt.1.and.(.not.pairpass)) then
c               do k=1,nfpass-1
c                  jf=jpass(k)
c
c                  if(debug) write(*,*) "Checking pair ", i," ", jf
c                  pairpass=.true.
c                  if(dSqrt(Sumdot(plab(0,i),plab(0,jf),+1d0)).lt.mmff)then
c                     if(debug) write(*,*) "mmff too small -> fails"
c                     pairpass=.false.
c                  elseif(dSqrt(Sumdot(plab(0,i),plab(0,jf),+1d0)).gt.mmffmax)then
c                     if(debug) write(*,*) "mmff too large -> fails"
c                     pairpass=.false.
c...  etot(ij) cut?
c                  elseif(plab(0,i)+plab(0,jf).lt.efTot) then
c                     if(debug) write(*,*) "efTot too small -> fails"
c                     pairpass=.false.
c                  else
c...  eratio(ij) cut?
c                     efRatVal=plab(0,i)/plab(0,jf)
c                     if(efRatVal.gt.1) efRatVal=1d0/efRatVal
c                     if(efRatVal.lt.efRat) then
c                       if(debug) write(*,*) "efRatio too small -> fails"
c                       pairpass=.false.
c                     endif
c                  endif
c               enddo
c            endif               ! Close pair criteria
c 666        continue
c         endif ! Close if is_a_f
c      enddo ! Close loop over final state particles

c... Check: were criteria satisfied?
...
cc...  Check: were inclusive criteria satisfied?      
c      if(.not. xgoodf) then
c         if(debug) write(*,*) "no f pass inclusive -> fails"
c         passcuts=.false.
c         return
c      endif
c
c      if(nfpass.lt.2) then
c         if(debug) write(*,*) "no two f pass 'exclusive' -> fails"
c         passcuts=.false.
c         return
c      endif
c
c      if(.not.pairpass) then
c         if(debug) write(*,*) "no two f pass 'pair' -> fails"
c         passcuts=.false.
c         return
c      endif
...
cccccccccccccccccccccccccc END FIXED TARGET CUTS ccccccccccccccccc
c     
c     p_t min & max cuts
c     
      do i=nincoming+1,nexternal
