...
export JOBDIR=$HPSMC/prod/slac/tritrig/slic
export RUNDIR=$SCRATCH/tritrig/slic/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -o $RUNDIR/../logs/job.${JOB_ID}.out -e $RUNDIR/../logs/job.${JOB_ID}.err -l $RUNDIR/../logs/job.${JOB_ID}.log -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} tritrig_prep_and_slic $JOBDIR/jobs.json
