...
def main():
    ...
    param_list = [[120,1e-3],[120,8e-4],[120,5e-4],[120,2e-4]]
    ...
    for mass_mev,epsilon in param_list:
        ...
        if bg is None or not hasattr(bg, "BACKGROUND_PATH"):
            ...
            center_geV = float(Mkey)/1000.0 #1.8*Mkey / (1000.0 * 3.0)
            print("Mass of Vd: "+str(mass_mev))
            print("Location of Center: "+str(center_geV))
            mass_mask = (invM > (center_geV - 0.002)) & (invM < (center_geV + 0.002))
            #(invM > -1)#(invM > (center_geV - 0.002)) & (invM < (center_geV + 0.002))
