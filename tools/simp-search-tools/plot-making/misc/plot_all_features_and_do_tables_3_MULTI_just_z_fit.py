...
def main():
    ...
    try:
        ...
        sys.exit(1)
    ##ALL OF THIS HAS BEEN VALIDATED SO FAR
    #"vertex.pos_.fX_","vertex.pos_.fY_","vertex.pos_.fZ_", 
    features_to_plot = ["psum","vertex.invM_","vertex.invMerr_",
    ...
    features_to_plot += ["vertex.pos_.fX", "vertex.pos_.fY", "vertex.pos_.fZ"]
 
    param_list = [[90,1e-2],[90,9e-3],[90,8e-3],[90,7e-3],[90,6e-3],[90,5e-3],[90,4e-3],[90,3e-3],[90,2e-3],[90,1e-3],[90,9e-4],[90,8e-4],[90,7e-4]]
    plotarray_data = [[[] for j in range(len(features_to_plot))] for i in range(len(param_list)+1)]
    plotarray_bins = [[0 for j in range(len(features_to_plot))] for i in range(len(param_list)+1)]
    plotarray_weights = [[[] for j in range(len(features_to_plot))] for i in range(len(param_list)+1)]
    plotarray_ranges = [[[] for j in range(len(features_to_plot))] for i in range(len(param_list)+1)]

     
    ##WHAT I WILL WANT TO DO IS LOOP OVER THE MASS AND EPSILON LIST (AND ONE FOR BACKGROUND], FOR EACH, STORE SDATA INTO THE RELEVANT FEATURE POSITION
    ##SO PLOTARRAY SHOULD BE DIMENSION [len(param_list)+1]X[feature_list]
    count=1
    for mass_mev,epsilon in param_list:
        ...
        if bg is None or not hasattr(bg, "BACKGROUND_PATH"):
            ...
            mass_mask = (invM > -1)#(invM > (center_geV - 0.002)) & (invM < (center_geV + 0.002))
        ...
        try:
            ...
            print("unboosted cut length: "+str(rho_length))
        ...
        for feat in features_to_plot:
            ...
            if feat in events:
                s_data = np.asarray(events[feat])[sig_final_mask]
            elif feat.startswith("vertex.pos_.") and ("vertex.pos_" in events):
                _fld = feat.split(".")[-1]
                s_data = np.asarray(events["vertex.pos_"][_fld])[sig_final_mask]
            else:
                continue  # feature not present in signal, skip

            # Background feature fetch (supports either split keys or record "vertex.pos_")
            if bg_final_mask is not None and (feat in arrays.keys()):
                b_data = ak.to_numpy(arrays[feat])[bg_final_mask]
            elif bg_final_mask is not None and feat.startswith("vertex.pos_.") and ("vertex.pos_" in arrays.keys()):
                _fld = feat.split(".")[-1]
                b_data = ak.to_numpy(arrays["vertex.pos_"][_fld])[bg_final_mask]
            else:
                b_data = np.array([]) 
            ...
            if(normalized):
                ...
                B_final = 1.0
            if s_data.size > 0:
                s_w = np.full(s_data.shape, S_final / s_data.size)
            if b_data.size > 0:
                b_w = np.full(b_data.shape, B_final / b_data.size)
            bins = args.bins
            if((count==1)):
                plotarray_data[0][count2].extend(b_data)
                plotarray_bins[0][count2]=bins
                plotarray_weights[0][count2].extend(b_w)
                plotarray_ranges[0][count2].extend(hist_range)
            plotarray_data[count][count2].extend(s_data)
            plotarray_bins[count][count2]=bins
            plotarray_weights[count][count2].extend(s_w)
            plotarray_ranges[count][count2].extend(hist_range)
            count2+=1
        count+=1     
    #NOW I CAN PLOT WITH THE ARRAYS
    I=len(features_to_plot)-1
    feat=features_to_plot[I]
    xmax_list = [6.0,6.0,6.0,6.0,6.0,6.0,6.0,20.0,20.0,25.0,25.0,25.0,25.0]
    slopelist=[]
    rho_list=[]
    phi_list=[]
    phi_frac_list=[]

    plt.hist(plotarray_data[0][I],bins=plotarray_bins[0][I],range=plotarray_ranges[0][I], weights=plotarray_weights[0][I],histtype="step",linewidth=1.8,label="Background")
    for J in range(len(param_list)):
        counts, edges, _ = plt.hist(plotarray_data[1+J][I],bins=plotarray_bins[1+J][I],range=plotarray_ranges[1+J][I], weights=plotarray_weights[1+J][I],histtype="step",linewidth=1.8,label="Mass: "+str(param_list[J][0])+", eps: "+str(param_list[J][1]))
        # --- 2) NOW do the log-linear fit + overlay ---
        centers = 0.5 * (edges[:-1] + edges[1:])
        y = counts.astype(float)

        xmax = float(xmax_list[J])   # array aligned with param_list
        mask = (centers >= 0.0) & (centers <= xmax) & (y > 0.0)
        ...
        logy = np.log(np.maximum(yfit, 1e-30))
        slope, intercept = np.polyfit(xfit, logy, 1)
        slopelist.append(slope)
        xline = np.linspace(0.0, xmax, 200)
        yline = np.exp(intercept + slope * xline)

        plt.plot(xline, yline, linestyle="--", linewidth=1.5)    
       
        mass_mev = param_list[J][0]
        epsilon = param_list[J][1] 
        alpha_D = 0.01  # fixed in decayLength7sel
        m_pi_D = mass_mev / 3.0
        m_V_D = 1.8*mass_mev / 3.0 
        # For maximum visible fraction, use f_pi_D such that invisible width is minimal (f_pi as in last rho array entry scenario)
        ...
        psum = 2.0
        gamma = 1000*psum/m_V_D
        rho_list.append(rho_length*gamma)
        phi_list.append(phi_length*gamma)
        phi_frac_list.append(phi_fraction)
    ...
    plt.close()
    plt.scatter([a[1] for a in param_list],slopelist,s=10,c="r",marker="o")
    plt.scatter([a[1] for a in param_list],[-1/a for a in rho_list],s=10,c="b",marker="o")
    plt.scatter([a[1] for a in param_list],[-1/a for a in phi_list],s=10,c="y",marker="o")
    plt.title("Slopes as a function of Epsilon")
    plt.ylabel("Slope")
    plt.xlabel("Epsilon")
    plt.savefig(os.path.join(args.outdir,"slopesplot.png"))
    plt.close()
