...
# ===== TAIL-PATCH NOTE =====
# This helper now supports both the original shell labels and a single-threshold tail mode.
def plot_shell_histogram(var_name, values, ann_mask, bdt_mask, ann_meta, bdt_meta, outpath, bins=60, density=True):
    ...
    # ===== TAIL-PATCH CHANGE START =====
    ann_mode = ann_meta.get("mode", "shell")
    bdt_mode = bdt_meta.get("mode", "shell") if bdt_meta is not None else None

    if ann_vals.size > 0:
        if ann_mode == "tail":
            ann_label = (f"ANN tail, target {ann_meta['target']:.1e}; "
                         f"realized {ann_meta['realized']:.3e}; "
                         f"N={ann_vals.size}")
        else:
            ann_label = (f"ANN shell, target {ann_meta['lo_target']:.1e}->{ann_meta['hi_target']:.1e}; "
                         f"realized {ann_meta['lo_realized']:.3e}->{ann_meta['hi_realized']:.3e}; "
                         f"N={ann_vals.size}")
        ax.hist(ann_vals, bins=edges, histtype='step', linewidth=2.0, density=density, label=ann_label)
        any_plotted = True

    if bdt_mask is not None and bdt_vals.size > 0:
        if bdt_mode == "tail":
            bdt_label = (f"BDT tail, target {bdt_meta['target']:.1e}; "
                         f"realized {bdt_meta['realized']:.3e}; "
                         f"N={bdt_vals.size}")
        else:
            bdt_label = (f"BDT shell, target {bdt_meta['lo_target']:.1e}->{bdt_meta['hi_target']:.1e}; "
                         f"realized {bdt_meta['lo_realized']:.3e}->{bdt_meta['hi_realized']:.3e}; "
                         f"N={bdt_vals.size}")
        ax.hist(bdt_vals, bins=edges, histtype='step', linewidth=2.0, density=density, label=bdt_label)
        any_plotted = True

    title_mode = ann_mode if ann_mode is not None else "shell"
    if title_mode == "tail":
        ax.set_title(f"Background tail comparison: {var_name}")
    else:
        ax.set_title(f"Background shell comparison: {var_name}")
    # ===== TAIL-PATCH CHANGE END =====
...
def main():
    ap = argparse.ArgumentParser(description="Plot ANN-vs-BDT background shell histograms between tiny retention bands, or a single retained tail.")
    ...
    # ===== TAIL-PATCH CHANGE START =====
    ap.add_argument("--tail-retention", type=float, default=None,
                    help="If set, make a single 34-plot comparison for the retained tail at this background fraction, e.g. 1e-4")
    # ===== TAIL-PATCH CHANGE END =====
    ...
    # ===== TAIL-PATCH CHANGE START =====
    if args.tail_retention is None:
        ...
        if args.tail_retention <= 0.0 or args.tail_retention >= 1.0:
            raise ValueError("--tail-retention must be between 0 and 1, e.g. 1e-4")
        r_tail = args.tail_retention
    # ===== TAIL-PATCH CHANGE END =====
    ...
    if args.tail_retention is None:
        ...
        ranges = [
            (
                ...
                ann_shell_loose_mid,
                bdt_shell_loose_mid,
                {"mode": "shell", "lo_target": r_loose, "hi_target": r_mid, "lo_realized": ann_real_loose, "hi_realized": ann_real_mid},
                {"mode": "shell", "lo_target": r_loose, "hi_target": r_mid, "lo_realized": bdt_real_loose, "hi_realized": bdt_real_mid},
            ),
            (
                f"{r_mid:.1e}_to_{r_tight:.1e}",
                ann_shell_mid_tight,
                bdt_shell_mid_tight,
                {"mode": "shell", "lo_target": r_mid, "hi_target": r_tight, "lo_realized": ann_real_mid, "hi_realized": ann_real_tight},
                {"mode": "shell", "lo_target": r_mid, "hi_target": r_tight, "lo_realized": bdt_real_mid, "hi_realized": bdt_real_tight},
            ),
        ]
    else:
        ann_thr_tail, ann_real_tail, ann_keep_tail, ann_nbase = score_threshold_for_retention(ann_scores, common_mask, r_tail)
        bdt_thr_tail, bdt_real_tail, bdt_keep_tail, bdt_nbase = score_threshold_for_retention(bdt_scores, common_mask, r_tail)

        ann_tail_mask = common_mask & (ann_scores >= ann_thr_tail)
        bdt_tail_mask = common_mask & (bdt_scores >= bdt_thr_tail)
        ...
        ranges = [
            (
                ...
                ann_tail_mask,
                bdt_tail_mask,
                {"mode": "tail", "target": r_tail, "realized": ann_real_tail},
                {"mode": "tail", "target": r_tail, "realized": bdt_real_tail},
            ),
        ]
    # ===== TAIL-PATCH CHANGE END =====
    ...
    with open(summary_path, "w") as sf:
        # ===== TAIL-PATCH FIX START =====
        if args.tail_retention is None:
            sf.write("Background shell histogram summary\n")
        else:
            sf.write("Background tail histogram summary\n")
        # ===== TAIL-PATCH FIX END =====
        ...
        if args.tail_retention is None:
            ...
            sf.write(f"  target tail_retention={r_tail:.3e}: threshold = {ann_thr_tail:.8g}, realized = {ann_real_tail:.8g}, requested keep ~= {ann_keep_tail}/{ann_nbase}\n\n")
        # ===== TAIL-PATCH FIX END =====
        ...
        if args.tail_retention is None:
            ...
            sf.write(f"  target tail_retention={r_tail:.3e}: threshold = {bdt_thr_tail:.8g}, realized = {bdt_real_tail:.8g}, requested keep ~= {bdt_keep_tail}/{bdt_nbase}\n\n")
        # ===== TAIL-PATCH FIX END =====
    ...
    if args.debug:
        ...
        if args.tail_retention is None:
            ...
            print(f"ANN realized retention: {ann_real_tail:.6e}")
            print(f"BDT realized retention: {bdt_real_tail:.6e}")
            print(f"ANN tail count: {np.count_nonzero(ann_tail_mask)}")
            print(f"BDT tail count: {np.count_nonzero(bdt_tail_mask)}")
