...
c A' decay distance and decay vertex
      real*8 dd, vx, vy, vz
c ctau in mm
      real*8 gamma, beta,ctau/10.0/
      real*4 rndm4
...
c sample A' decay distance and save decay vertex
      if(idup.eq.622) then
         p = sqrt(pup(4)**2 - pup(5)**2)
         call randomset(rndm)
         rndm4 = rndm
c sample exponential decay
         gamma = pup(4)/pup(5)
         beta = sqrt(1.-1./gamma**2)
         dd = -gamma*beta*ctau*alog(rndm4)
         vx = dd * pup(1)/p
         vy = dd * pup(2)/p
         vz = dd * pup(3)/p
      end if
...
c     reject non-final particles, non-EGS particles
      ...
      if ((idup.eq.611).or.(idup.eq.-611)) then
c save e+/e- from A'
            nhep = nhep + 1
