...
!  3) Theta_x > 2mrad
!
!  THIS VERSION IS A MODIFIED beam_v5, 
!  WITH WIDE-ANGLE BREMSSTRAHLUNG SUPPRESSED
!  TO PREVENT DOUBLE-COUNTING WHEN USED ALONGSIDE MADGRAPH WABS
!
!  It is meant for mixing with 1.056 and 2.3 GeV WABs with a 50MeV
!  energy cut
!
!
!------------------------------- main code -----------------------------
...
! ----------------------------------------------------------------------
      ...
      integer iqsave, iskip
      real*8 esave, usave, vsave, wsave
...
c after brems call
      if(iarg.eq.7) then
         ...
         iskip = 0
        if(iq(np).eq.0.and.e(np).gt.400.0.and.abs(v(np)).gt.0.005)
     -      iskip = 1
        if(iq(np-1).eq.0.and.e(np-1).gt.400.0.and.abs(v(np-1)).gt.0.005)
     -      iskip = 1
...
cx      end if
      ...
      if(ir(np).eq.3) then
         iout = 1
         if(iq(np).eq.0.and.abs(v(np)).lt.0.004) iout = 0
         if(iq(np).eq.-1) then
            if(e(np).lt.ebeam*0.005) iout = 0
            if(e(np).gt.ebeam*0.6.and.
     -         sqrt(u(np)**2+v(np)**2).lt.0.005) iout = 0
         end if
