...
c after brems call
      if(iarg.eq.7) then
         ...
         iskip = 0
         if(iq(np).eq.0.and.e(np).gt.800.and.abs(v(np)).gt.0.005)
     -      iskip = 1
         if(iq(np-1).eq.0.and.e(np-1).gt.800.and.abs(v(np-1)).gt.0.005)
     -      iskip = 1
