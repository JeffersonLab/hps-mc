...
************************************************************************
     ...
     . kDp(nincoming+1:nexternal),ksDp(nincoming+1:nexternal)
      ...
      double precision mu_k,mu_ij,qtot(0:3),qsq,pij(0:3),lambda,msq_ij
...
c---  final-final
       ...
       qsq=dot(qtot,qtot)
         do j=1,nexternal
            do nu=0,3
               if (j.eq.ip) then
                 q(nu,ipart)=qtot(nu)-(sqrt(lambda(qsq,msq_ij,msq_k))/sqrt(lambda(qsq,dot(pij,pij),msq_k))
...
c                  print*, msq_ij,msq_k,dot(pij,pij),(2d0*dot(p(0,ip),p(0,jp)))
                  q(nu,ipart)=sqrt(lambda(qsq,msq_ij,msq_k))/sqrt(lambda(qsq,dot(pij,pij),msq_k))
     &                        *(p(nu,kp)-dot(qtot,p(0,kp))/qsq*qtot(nu))
     &                        +(qsq+msq_k-msq_ij)/two/qsq*qtot(nu)
                  if (ipart.ne.kpp)then
                     write(*,*)'ERROR, problem with momentum mapping ff'
