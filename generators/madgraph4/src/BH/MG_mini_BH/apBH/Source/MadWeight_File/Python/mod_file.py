...
##     2) we apply the modification for $B$ NAME $E$                                              ##
##     3) we apply the modification for $B$ NAME $B$ TEXT $E$ NAME $E$                            ##
##     4) we convert the $ ?-DIFF$ in normal tag                                                  ##
##                                                                                                ##
####################################################################################################
##                                                                                                ##
## Exemple of use:                                                                                ##
...
##    import mod_file                                                                             ##
##                                                                                                ##
##    mod_file.mod_file(file_to_change,rule,[output=''])                                          ##
...
#        print "dico", self.dico.keys()
        ...
        begin_end=re.compile(r'''\$(?P<maj>B)\$\s?(?P<tag>\S+)\s?\$B\$(?P<text>.*)\$E\$\s?(?P=tag)\s?\$E\$''',re.S)    # $B$ TAG $B$ TEXT $E$ TAG $E$
        end_file=re.compile(r'''\$\$\s*END\s+FILE\s*\$\$''')     # $$ END FILE $$
        replace=re.compile(r'''\$(?P<maj>B)\$[ \t]*(?P<tag>\S+)[ \t]*\$E\$''',re.I)  # $B$ TAG $E$
        end_begin=re.compile(r'''\$(?P<maj>E)\$\s?(?P<tag>\S+)\s?\$E\$(?P<text>.*)\$B\$\s?(?P=tag)\s?\$B\$''',re.S)    # $E$ TAG $E$ TEXT $B$ TAG $B$ -> in case of multiple tag

        ##first treat global regexp
        for key in self.dico.keys():
            if key[:8]=='S-REGEXP':
                text=self.return_mod_text(key,text)
                
        ## fisrt treat replacment
    ...
    def treat_multiple_tag(self,text,maj,tag):
        end_begin=re.compile(r'''\$E\$\s?(?P<tag>\S+)\s?\$E\$(?P<text>.*)\$(?P<maj>B)\$\s?(?P=tag)\s?\$B\$''',re.S)    # $E$ TAG $E$ TEXT $B$ TAG $B$ -> in case of multiple tag
