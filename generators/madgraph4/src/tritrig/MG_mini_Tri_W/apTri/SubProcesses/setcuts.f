...
c-charged-leptons
         ...
         if (abs(idup(i,1)).eq.611)  is_a_l(i)=.true. ! f+ f-
c- F distinguished leptons
         if (abs(idup(i,1)).eq.611)  is_a_f(i)=.true. ! f+ f-
         if (abs(idup(i,1)).eq.11)  is_a_f(i)=.true. ! f+ f-  COUNT E TOO!
c- F distinguished leptons -- distinguish charge!
         if (idup(i,1).eq.611)  is_a_fm(i)=.true. ! f-
         if (idup(i,1).eq.11)  is_a_fm(i)=.true. ! e-  COUNT E TOO!
         if (idup(i,1).eq.-611)  is_a_fp(i)=.true. ! f+
         if (idup(i,1).eq.-11)  is_a_fp(i)=.true. ! e+  COUNT E TOO! 
c-b-quarks
...
c               if( is_a_f(i).and.is_a_f(j) ) s_min(j,i)=0.000 ! disable here
...
c               if(is_a_f(i).and.is_a_f(j) ) s_max(j,i)=10000.000 ! disable here
               if((is_a_l(i).and.is_a_l(j)).and.
     &            (abs(idup(i,1)).eq.abs(idup(j,1))).and.
     &            (idup(i,1)*idup(j,1).lt.0)) 
