...
class Level_generation:
    ...
    def define_fuse_region(self,fuse_list):
        ...
        while 1:
            if len(fuse_list)==1:
                ...
                part1=fuse_list.pop(0)
                part2=fuse_list.pop(0)
                ...
                self.num_fuse+=1
                fuse_list.append(fuse_particle)
...
class Blob_solution(Level_generation):
    ...
    def find_all_solutions(self,blob_sec):
        ...
        if self.opt.use_sol_type_3:
            ...
            blob_sec.solution.remove(self)# don't remove completly this solution it's the 'generic' one for the other
...
class Block_sector:
    ...
    def __init__(self,sol_sec,chgt_var,input_propa,output_propa):
        ...
        if type(output_propa)==list: #output_propa must be a list, but sometimes it's forget
...
class Block_ECS(Block_sector):
    ...
    def def_order_content(self):
        """ define (in the corect order) the particle used in this ECS
            This routine is overwritted for block linked to blob
        """
...
class Block_B(Block_sector):
    ...
    def def_order_content(self):
        ...
        elif(type(self.in_part[0].MG)==str): #check if first particle are fuse particle
            self.order_content=self.in_part+[self.in_part[0].mother]
            try:
                self.order_content.append(self.in_part[2].mother)
            except:
                pass
        elif(type(self.in_part[1].MG)==str): #check if first particle are fuse particle (symetric case)
