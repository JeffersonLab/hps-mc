...
class MG_diagram(diagram):
    ...
    def organize_particle_content(self,param_card,tf_file):
        ...
        process_tag=re.compile(r'''MW_P(?P<tag>\d*)''')
        cond=process_tag.search(self.directory).group('tag')
        proc_decay=Decay_info(self.directory+'/../../Cards/proc_card.dat',cond)
    ...
    def import_ext_part(self,dir_file):
        ...
        #leshouche.inc ->give PID and MG number
        ff=open(dir_file+'/'+'leshouche.inc','r')
        while 1:
            line=ff.readline()
            if line=='':
                sys.exit('incorrect leshouche.inc in '+dir_file+' directory') 
            if pid_pat.search(line):
                pid_list=pid_pat.search(line).group('pid')
                pid_list=space_pat.split(pid_list) #pass to vector form
                ff.close()
                break
    ...
    def import_process(self,dir_file,config):
        ...
        ff=open(dir_file+'/python_data.txt','r')
    ...
    def define_tf_width(self,file):
        ...
        dico_pid_to_type={-11:'e',11:'e',
              -13:"m",13:"m",2000013:"m",-2000013:"m",
              -15:"t",15:"t",
              -1:"j",-2:"j",-3:"j",-4:"j",1:"j",2:"j",3:"j",4:"j",
              -5:"b",5:"b",
              21:"g",
              22:"a"}
#        dico={"e":[-11,11],"m":[-13,13],'t':[-15,15],'a':[22],'j':[-4,-3,-2,-1,1,2,3,4],'b':[-5,5],'g':[21]}
        ...
        for part in self.ext_content:
            if not part.neutrino:
                part.tf_level=dico_type_to_tflevel[dico_pid_to_type[part.pid]]
            else:
                part.tf_level=3
    ...
    def detect_invisible_propa(self):
        ...
        for neut in  self.neut_content:
            if neut.twin in self.neut_content:
                ...
                neut.mother.neutrino=1
                neut.mother.level+=-1 #pass from propa to external definition
                #put standard info of external part:
                neut.mother.tf_level=3
    ...
    def check_decay(self,particle_list,decay_list):
        ...
        #print 'pass in step 2:',len(particle_list),'=?=',len(decay_list)
        #step 2:
        particle_list_pid=[]
...
class Option:
    ...
    def __init__(self,info='default'):
       ...
       if type(info)==str and info!='default':
       ...
       for couple in info['mw_gen'].items():
           couple=list(couple)
           if couple[1]=="T":
               couple[1]='1'
           elif couple[1]=="F":
               couple[1]='0'
           exec(tag_to_genvar[couple[0]]+'='+couple[1])


       if info['mw_run'].has_key('3'):
           if info['mw_run']['3']=="T":
               self.use_stat=1
...
if(__name__=="__main__"):
    ...
    os.chdir('..')
    for i in range(1,2):
        diag=MG_diagram('./SubProcesses/MW_P_mu+mu-_mu+mu-e+e-/','param_card.dat','./Source/Transfer_Fct/ordering_file.inc',i)
