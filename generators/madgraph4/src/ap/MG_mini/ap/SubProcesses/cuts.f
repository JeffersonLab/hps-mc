...
C $B$ DESACTIVATE_CUT $E$ !This is a tag for MadWeight
      ...
      if(debug) write (47,*) '============================='
      if(debug) write (47,*) ' EVENT STARTS TO BE CHECKED  '
      if(debug) write (47,*) '============================='
...
c     
      do i=nincoming+1,nexternal
         if(debug) write (47,*) 'pt(',i,')=',pt(plab(0,i)),'   ',etmin(i),':',etmax(i)
         notgood=(pt(plab(0,i)) .le. etmin(i)).or.
     &        (pt(plab(0,i)) .gt. etmax(i))
         if (notgood) then
            if(debug) write (47,*) i,' -> fails'
            passcuts=.false.
            return
         endif
      enddo
...
c-  check the et
     ...
     &        (pt(ptemp(0)) .gt. missetmax)
         if (notgood) then
            if(debug) write (47,*) ' missing et cut -> fails'
            passcuts=.false.
            return
         endif
      endif
...
c
      if(ptheavy.gt.0d0)then
         ...
         if(.not.passcuts.and.foundheavy)then
            if(debug) write (47,*) ' heavy particle cut -> fails'
            return
         else
            passcuts=.true.
         endif
...
c     
      do i=nincoming+1,nexternal
         if(debug) write (47,*) 'plab(0,',i,')=',plab(0,i),'   ',emin(i),':',emax(i)
...
c     
      do i=nincoming+1,nexternal
         if(debug) write (47,*) 'abs(rap(',i,'))=',abs(rap(plab(0,i))),'   ',etamin(i),':',etamax(i)
...
c     
      do i=nincoming+1,nexternal-1
         do j=i+1,nexternal
            if(debug) write (47,*) 'r2(',i, ',' ,j,')=',dsqrt(r2(plab(0,i),plab(0,j)))
            if(debug) write (47,*) dsqrt(r2min(j,i)),dsqrt(r2max(j,i))
            if(r2min(j,i).gt.0.or.r2max(j,i).lt.1d2) then
               tmp=r2(plab(0,i),plab(0,j))
               notgood=(tmp .lt. r2min(j,i)).or.(tmp .gt. r2max(j,i))
               if (notgood) then
                  if(debug) write (47,*) i,j,' -> fails'
                  passcuts=.false.
                  return
               endif
            endif
...
c     
      do i=nincoming+1,nexternal-1
         do j=i+1,nexternal
            if(debug) write (47,*) 's(',i,',',j,')=',dsqrt(Sumdot(plab(0,i),plab(0,j),+1d0))
            if(debug) write (47,*) dsqrt(s_min(j,i)),dsqrt(s_max(j,i))
...
c- fill ptjet with the pt's of the jets.
      ...
      enddo
      if(debug) write (47,*) 'not yet ordered ',njets,'   ',ptjet
...
c- check existance of jets if jet cuts are on
     ...
     $     njets.lt.4.and.ptj4min.gt.0)then
         if(debug) write (47,*) i, ' too few jets -> fails'
         passcuts=.false.
         return
      endif
...
c - sort jet pts
      do i=1,njets-1
         ...
         enddo
      enddo
      if(debug) write (47,*) 'ordered ',njets,'   ',ptjet
c
...
c     
      if(njets.gt.0) then
       ...
       notgood=.not.jetor
       if(debug) write (47,*) 'jetor :',jetor  
       if(debug) write (47,*) '0',notgood   
      
      do i=1,njets 
            if(debug) write (47,*) i,ptjet(i), '   ',ptjmin4(min(i,4)),':',ptjmax4(min(i,4))
         if(jetor) then     
c---  if one of the jets does not pass, the event is rejected
     ...
     $           (ptjet(i).lt.ptjmin4(min(i,4)))
            if(debug) write (47,*) i,' notgood total:', notgood   
         else
c---  all cuts must fail to reject the event
     ...
     $              .or.(ptjet(i).lt.ptjmin4(min(i,4))))
      ...
      if (notgood) then
         if(debug) write (47,*) i, ' multiple pt -> fails'
         passcuts=.false.
         return
      endif
...
C---------------------------
      ...
      do i=2,njets
         htj=htj+ptjet(i)
         if(debug) write (47,*) i, 'htj ',htj
         if(debug) write (47,*) 'htmin ',i,' ', htjmin4(min(i,4)),':',htjmax4(min(i,4))
         if(htj.lt.htjmin4(min(i,4)) .or. htj.gt.htjmax4(min(i,4))) then
            if(debug) write (47,*) i, ' ht -> fails'
            passcuts=.false.
            return
         endif
      enddo
...
c     Set couplings in model files
      ...
      if(debug) write (47,*) '============================='
      if(debug) write (47,*) ' EVENT PASSED THE CUTS       '
      if(debug) write (47,*) '============================='
